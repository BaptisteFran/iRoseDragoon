			...
			if ( szFileName ) 
			{
				...
				}else
				{
					...
					if( iImageZoneNO > 500 )
					{
						...
						m_LoadingImageTableByPlanet.insert( std::make_pair( iImageZoneNO - 500,  std::string(szFileName) ) );
					}else
					{
						/// 존
						m_LoadingImageTableByZone.insert( std::make_pair( iImageZoneNO,  std::string(szFileName) ) );
					}
				}
			}else
			{
