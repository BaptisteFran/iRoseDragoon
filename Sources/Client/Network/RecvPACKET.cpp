	...
	while( szServerName && pServerID ) {
		...
		if( g_GameDATA.m_bForOpenTestServer )
		{
			if( szServerName[0] == '@' && strlen(szServerName) >= 2 )
				TempServerList.insert( std::map< BYTE, std::pair< DWORD, std::string> >::value_type( (BYTE)( 128 + (*pServerID) ), std::make_pair( *pServerID, &szServerName[1] ) ) );
		}
		else
		{
			if( szServerName[0] == '@')
			{  
				if( dwRight >= CHEAT_MM) 
				{
					if( strlen(szServerName) >= 2 )
						TempServerList.insert( std::map< BYTE, std::pair< DWORD, std::string> >::value_type( (BYTE)( 128 + (*pServerID) ) , std::make_pair( *pServerID, &szServerName[0] ) ) );
				}
			}
			else
			{
				if( strlen(szServerName) >= 2 )
					TempServerList.insert( std::map< BYTE, std::pair< DWORD, std::string> >::value_type( szServerName[0], std::make_pair( *pServerID, &szServerName[1] ) ) );
