...
void IT_MGR::DrawDLGs( POINT ptMouse )
{
	...
	for_each(
		m_Dlgs.rbegin(),
		m_Dlgs.rend(),
		[&ptMouse](CTDialog *dialog)
		{
			dialog->Update(ptMouse);
		});
	...
	for_each( m_Icons.begin(), m_Icons.end(), std::mem_fn( &CIcon::Draw ));
	for_each( m_Dlgs.begin(), m_Dlgs.end(), std::mem_fn( &CTDialog::Draw ));
