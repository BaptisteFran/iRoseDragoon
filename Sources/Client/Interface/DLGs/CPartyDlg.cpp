...
void CPartyDlg::Draw()
{
	...
	for_each( m_listPartyMember.begin(), m_listPartyMember.end(), std::mem_fn( &CPartyMember::Draw) );
...
void CPartyDlg::Update( POINT ptMouse )
{
	...
	for_each( m_listPartyMember.begin(), m_listPartyMember.end(), std::bind( std::mem_fn( &CPartyMember::Update), std::placeholders::_1, ptMouse ));
