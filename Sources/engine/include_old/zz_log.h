...
public:
	...
	*/
	...
	// do message log
	// exam) log("error : file not found (%s).₩n", filename);
...
#define ZZ_LOG_ERROR(message) ZZ_LOG("error : " #message " from [%s#%d]₩n", __FILE__, __LINE__)
#else
...
// usage :
// ...
// ZZ_LOG("error at line(%d) of file(%s)₩n", __LINE__, __FILE__);
// ...
