...
public:
	...
	{
		std::for_each( m_Sequences.begin(), m_Sequences.end(), std::mem_fn( &zz_particle_event_sequence::DeleteAllParticles) );		
		return 0;
	}
	...
	int Reset(void) 
	{
		std::for_each( m_Sequences.begin(), m_Sequences.end(), std::mem_fn( &zz_particle_event_sequence::Reset) );		
		return 0;
	}
