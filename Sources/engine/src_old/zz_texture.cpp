...
#ifdef LOG_TEXTURE
	ZZ_LOG("texture: create_texture(%s-%dx%d, %d)₩n",
...
#ifdef LOG_TEXTURE
	ZZ_LOG("texture: destroy_texture1(%s-%dx%d, %d)₩n",
...
bool zz_texture::set_path (const char * path_in)
{
	...
	zz_assert(path_in);
	zz_assertf(znzin->file_system.exist(path_in), "[%s] file not found₩n", path_in);
