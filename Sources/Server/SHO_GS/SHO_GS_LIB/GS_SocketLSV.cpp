		...
		else
		{
			if(!strcmpi(pToken, "/get"))
			{
				...
				if(pUSER)
				{
					...
					m_TmpSTR.Printf(
						"/w %s>HP:%d/%d, MP:%d/%d, LEV:%d, EXP:%d, JOB:%d, "
						"SPD(M:%d,A:%d), Hit:%d, Crt:%d, AP:%d, DP:%d, AVD:%d, "
						"ATTR:%d, bCST:%d, SKL:%d, FLG:%x, Summon:%d/%d, BP:%d, "
						"SP:%d, \\:%I64d",
						pUSER->Get_NAME(),
						pUSER->Get_HP(),
						pUSER->Get_MaxHP(),
