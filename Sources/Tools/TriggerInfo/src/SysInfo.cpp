...
void CSysInfo::GetOSInfo (void)
{
	...
	m_mapOS[ "Language" ] = (std::string)m_system->m_szLanguagesEnglish; // 현재 언어.
...
//	m_mapVideo[ "DirectX Version" ] = strDxVersion + "(" + directxversion + ")";
	...
	m_mapVideo[ "GraphicCardMemory" ] = (std::string)m_display1.m_szDisplayMemoryLocalized; // 그래픽카드 메모리 정보.
	m_mapVideo[ "DisplayMode" ] = (std::string)m_display1.m_szDisplayModeLocalized; // 현재 디스플레이 정보.
	m_mapVideo[ "DirectX Version" ] = (std::string)m_system->m_szDirectXVersionLongEnglish; // 다이렉트 X 버젼 정보.
}
