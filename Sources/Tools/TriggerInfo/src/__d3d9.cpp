...
bool CD3D9::GetDisplayInfo (std::map<std::string, std::string> * mapDisInfo)
{
	...
	if(mapDisInfo)
	{
		...
		if(s_pID3D)
		{
			...
			if(hRes	== S_OK)
			{
				mapDisInfo->insert (std::pair<std::string, std::string>("Driver"			, DAI.Driver));
				mapDisInfo->insert (std::pair<std::string, std::string>("Divice Name"		, DAI.DeviceName));
				mapDisInfo->insert (std::pair<std::string, std::string>("Description"		, DAI.Description));
				...
				mapDisInfo->insert (std::pair<std::string, std::string>("Drive Version"	, strDrvVersion));
			}
		}
	}
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CD3D9::GatherDXVA_DeinterlaceCaps(
	IDxDiagContainer* pParent, 
